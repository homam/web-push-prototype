...
self.addEventListener("message", e => {
  ...
  switch (data.type) {
    case 'retrieve-client-id':
      return e.source.postMessage(JSON.stringify({ type: 'client-retrieve-client-id:response', client_id: `${e.source.id}` }));
      break;
  }
})
...
self.addEventListener('notificationclick', ev => {
  ...
  post("/api/v1/analytics/push-clicked", { message_uuid, now: new Date(), action: action.id || action })
    .catch(() => { }) // eat the error
